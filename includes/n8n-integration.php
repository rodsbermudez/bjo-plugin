...
function patropi_bjo_send_references_shortcode() {
	...
	if ( $http_code >= 200 && $http_code < 300 ) {
		...
		if ( current_user_can( 'manage_options' ) && ! empty( $response_body ) ) { 
			...
			if ( json_last_error() === JSON_ERROR_NONE && isset( $citation_data['total_citacoes'], $citation_data['lista_citacoes'] ) ) {
				
				$success_message .= '<div class="patropi-n8n-citations-result">';
				$success_message .= '<h3>Encontradas ' . esc_html( $citation_data['total_citacoes'] ) . ' citações:</h3>';

				if ( ! empty( $citation_data['lista_citacoes'] ) ) {
					$success_message .= '<ul class="lista-citacoes-n8n">';
					foreach ( $citation_data['lista_citacoes'] as $citation ) {
						$referencia_formatada = $citation['referencia_formatada'] ?? 'Referência não disponível';
						$doi                  = $citation['doi'] ?? null;

						// Transforma o markdown-like **bold** em <strong> tags.
						$referencia_formatada = preg_replace( '/\*\*(.*?)\*\*/', '<strong>$1</strong>', $referencia_formatada );

						$success_message .= '<li>';
						$success_message .= '<p class="citation-reference">' . wp_kses_post( $referencia_formatada ) . '</p>';
						if ( $doi ) {
							$success_message .= '<a href="https://doi.org/' . esc_attr( $doi ) . '" target="_blank" rel="noopener noreferrer" class="button button-secondary">Ver arquivo</a>';
						}
						$success_message .= '</li>';
					}
					$success_message .= '</ul>';
